...
class FullReportResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        // For sensitive reports, check if the viewer has a pending or approved claim.
        // Admins and the report owner always see the real URLs.
        $canSeeImages = !$this->is_sensitive
            || $isAdmin
            || $isOwner
            || (
                $viewer
                && $this->relationLoaded('claims')
                && $this->claims
        ...
        return [
            ...
            'images' => $this->whenLoaded('images', function () use ($canSeeImages) {
                return $this->images->map(fn($img) => [
                    'id'  => $img->id,
                    // Real URL only for authorised viewers; 'PROTECTED' otherwise.
                    'url' => $canSeeImages ? $img->image_url : 'PROTECTED',
                ]);
            }),
